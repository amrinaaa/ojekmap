...
function App() {
    ...
    const getRoute = async () => {
      ...
      if (startCoords && endCoords && mapRef.current) {
        ...
        routeControl.on('routesfound', function (e) {
          ...
          const averageSpeedKmPerHour = 40; // Kecepatan rata-rata dalam km/jam
  ...
  return (
    <>
      <Navbar /> {/* Navbar dipanggil di sini */}
      <div className="flex justify-center items-center h-screen bg-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-8 rounded-lg shadow-lg">
                ...
                {!awal && <div id='keterangan' className="ml-10">
                  {distance && estimatedTime && (
                    <p className="mt-5 text-gray-700">
                      ...
                      Harga: Rp. {(Math.ceil(distance.toFixed(2) * 3000)).toLocaleString('id-ID')},- *<br />
                      <small>*Dibawah 1 KM = Rp. 10.000</small>
                    </p>
                  )}
                </div>}
              </div>
          ...
          <div className="flex items-center justify-center">
            {awal ? (
              <p className="text-red-500 text-2xl font-bold mb-5">Silahkan Isi Alamat Terlebih Dahulu</p>
            ) : (
              isLoading ? (
                <div className="flex items-center justify-center">
